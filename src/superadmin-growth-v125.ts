...
function creatorRow(creator: CreatorMetric) {
  ...
  subscriptions.innerHTML = `<strong>${Number(creator.activeSubscribers || 0)}</strong><span>assinantes ativos</span>`;

  const communities = document.createElement("div");
  communities.className = "superadmin-creator-stat-v125";
  communities.innerHTML = `<strong>${Number(creator.communityCount || 0)}</strong><span>comunidades</span>`;
...
function renderGrowthPanel() {
  ...
  page.querySelector(".superadmin-growth-v125")?.remove();
